...
export const SearchItem: React.FC<SearchItemProps> = ({ device }) => {
  ...
  return (
    <div className="grid grid-cols-[300px_1fr_200px] mb-2 border-gray-500/30 py-8 relative">
      <Link
        to={`/device/${id}`}
        className="place-items-center place-self-center"
      >
        <img
      ...
      <div>
        ...
        <div className="flex items-center mb-4">
          <Link to={`/device/${id}`} className="flex mr-2 items-center group">
            ...
            <div className='group-hover:text-blue-500'>{avgRating(device.reviews)}</div>
          </Link>
          <Link to={`/device/${id}`} className="flex mr-2 items-center group">
            <AiFillMessage className="w-5 h-5 mr-1 group-hover:fill-blue-500" />
            <div className='group-hover:text-blue-500'>{device.reviews.length} Отзывов</div>
          </Link>
      ...
      <div className="self-end place-items-end">
        ...
        >
          <BiBookmark
            className={`w-6 h-6 hover:fill-blue-500 fill-${
              isFavorite ? 'blue-500' : 'gray-500'
            }`}
          />
        </button>
